...
- architecture: removed `commands/` — the plugin ships no slash commands (the `/malka` wrapper was added and dropped within this unreleased cycle, so it never shipped). Malka is reached by natural-language auto-routing from her agent `description`, or by naming `malka-orchestrator` explicitly; this matches the jspsych plugin, where Tzadok has no command either.
