...
| **scaffold / clone** | building a folder's canonical set from its templates / duplicating an existing folder | the domain's `rules.md`, `coding-knowledge/02-analysis/references/smart_clone.md` |
| **domain** | one numbered folder under `coding-knowledge/`, holding one top-level project part's `rules.md`, the templates that build that folder, and its craft under `references/` and `assets/`. Four cover the project parts — `01-preprocessing`, `02-analysis`, `03-models`, `04-simulations` — and `00-constitution` governs none of them, so it holds the two rules files read on every job | `references/knowledge-index.md` |
