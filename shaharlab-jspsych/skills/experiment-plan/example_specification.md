...
The per-trial `window_status` / `window_left_ms` columns are the primary signal; the audit rows carry the event-level detail behind them. Recording and reporting rules live in the `jspsych-window-monitoring` skill.
...
- Custom window-monitoring helper: built per the `jspsych-window-monitoring` skill — `fullscreenchange` / `visibilitychange` / `blur` listeners write `attention_event` rows, focus/visibility returns close the away interval, and every trial is stamped with `window_status` / `window_left_ms` (columns listed in Data Variables)
