...
The per-trial `window_status` / `window_left_ms` columns are the primary signal; the audit rows carry the event-level detail behind them. Recording and reporting rules live in the `jspsych-coding-style` skill's
`references/validity_checks/window-monitoring.md`.
...
- Custom window-monitoring helper: built per the `jspsych-coding-style` skill's `references/validity_checks/window-monitoring.md` — `fullscreenchange` / `visibilitychange` / `blur` listeners write `attention_event` rows, focus/visibility returns close the away interval, and every trial is stamped with `window_status` / `window_left_ms` (columns listed in Data Variables)
