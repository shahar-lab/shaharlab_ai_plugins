...
Data footprint: when **leaving the window** is used, every trial row carries two extra columns — `window_status` (`ok` / `left`) and `window_left_ms` (time away during that trial) — so include them in Section 4's example table and column guide. The `jspsych-coding-style` skill's [validity_checks/window-monitoring.md](../jspsych-coding-style/references/validity_checks/window-monitoring.md) reference defines how these columns are recorded and reported.
